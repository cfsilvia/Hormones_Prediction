...
class plot_data:
    ...
    def order_results(self,hormones, end_cann, aminoacids):
        set = pd.DataFrame(columns = self.results.columns)
       
        #data_names = hormones + end_cann + aminoacids
        data_names = end_cann
        self.ordered_results = self.results.set_index("hormone").loc[data_names].reset_index()
        for data in (aminoacids, end_cann, hormones):
    ...
    def plot_bar_plots(self):
        ...
        fig, (ax_left, ax_labels,  ax_right)  = plt.subplots(1, 3, figsize = (8,11), sharey = False, 
                                                gridspec_kw = {"width_ratios": [4, 1.2, 4], "wspace": 0.05})
        
        for i in range(len(hormones)):
            ax_left.barh(y[i], male_means[i], xerr = male_sem[i], color ="limegreen", alpha = alphas[i], capsize =4, error_kw={"elinewidth":1, "capthick":1,"zorder": 3})
            ax_right.barh(y[i], female_means[i], xerr = female_sem[i], color ="mediumpurple", alpha = alphas[i], capsize =4, error_kw={"elinewidth":1, "capthick":1,"zorder": 3})
        ...
        ax_left.set_xlim(max_val * margin, 0)   # left plot reversed
        ax_right.set_xlim(0, max_val * margin)
    ...
    def plot_bar_plots_broken_axis(self):
        ...
        gs = GridSpec(1, 3, width_ratios=[4, 0.5, 4], wspace=0.05)

        # Broken axes with different scales on each side
        # Left (male): mirrored so bars extend to the LEFT
        ax_left = brokenaxes(
            xlims=((HIGH[1], HIGH[0]), (LOW[1], LOW[0])),
            wspace=0.05, subplot_spec=gs[0, 0]
        )
        # Labels column
        ax_labels = fig.add_subplot(gs[0, 1]); ax_labels.axis("off")
        # Right (female): normal ascending x
        ax_right = brokenaxes(
            xlims=((LOW[0], LOW[1]), (HIGH[0], HIGH[1])),
            wspace=0.05, subplot_spec=gs[0, 2]
        )
        ...
        for i in range(len(hormones)):
            ax_left.barh(
                y[i], male_means[i], xerr=male_sem[i],
                color="mediumaquamarine", capsize=4,
                error_kw={"elinewidth":1, "capthick":1, "zorder":3},
                height=0.5
            )
            ax_right.barh(
                y[i], female_means[i], xerr=female_sem[i],
                color="mediumpurple", capsize=4,
                error_kw={"elinewidth":1, "capthick":1, "zorder":3},
                height=0.5
            )

        # Align y across containers
        ax_left.set_ylim(-0.5, len(hormones) - 0.5)
        ax_right.set_ylim(-0.5, len(hormones) - 0.5)

            # ---- Put x-axes at TOP and keep them visible
          # ---- Force the x-axis to the TOP using twiny() for each broken segment
        def force_top_ticks(bax, label, ticksize=8, labelsize=10, pad=2, labelpad=8):
        # Hide bottom ticks/labels on originals, and overlay a top-twin axis
         for a in bax.axs:
            # bottom off
            a.tick_params(axis="x", bottom=False, labelbottom=False)
            a.spines["bottom"].set_visible(False)

            # make a twin x-axis at the top
            at = a.twiny()
            at.set_xlim(a.get_xlim())  # keep range identical to the segment
            at.tick_params(axis="x", top=True, labeltop=True,
                           labelsize=ticksize, pad=pad)
            # keep only the top spine on the twin
            at.spines["top"].set_visible(True)
            at.spines["bottom"].set_visible(False)
            at.spines["left"].set_visible(False)
            at.spines["right"].set_visible(False)
            at.set_zorder(10)

         # Put the “axis label” as a panel title on top (reliable with brokenaxes)
         bax.set_title(label, pad=labelpad, fontsize=labelsize)
            
        force_top_ticks(ax_left,  "Male (pg/mg)")
        force_top_ticks(ax_right, "Female (pg/mg)")

        


        # Zero reference line on the left container
        for a in ax_left.axs:
        # only draw where 0 is within this sub-axes limits
          xmin, xmax = a.get_xlim()
          if min(xmin, xmax) <= 0 <= max(xmin, xmax):
            a.axvline(0, color="black", lw=1, zorder=2)
        ...
        for spine in ax_labels.spines.values():
         ...
         # Hide y ticks/labels everywhere (center column has the names)
        for a in ax_left.axs + ax_right.axs:
            a.set_yticks([]); a.set_yticklabels([])
            
        self.add_scatter_points(ax_left, ax_right, hormones)
        # Leave headroom for the top axes (prevents clipping)
        plt.tight_layout()
        plt.subplots_adjust(top=2)  # ↓ move down if anything is still clipped

        plt.savefig("F:/SilviaData/rutiFrishman/September2025/hormones_plot_b.pdf",
                    bbox_inches="tight")
        plt.close()
    #-------------------------------------------
    ...
    def __call__(self,hormones, end_cann, 
        ...
        self.order_results(hormones, end_cann, 
                       aminoacids)
        self.plot_bar_plots()

...
def robust_archetypes(X, k, n_iter=20, sample_frac=0.8, tol=1e-4, verbose=True):
    ...
    hull_verts = hull_full.points[hull_full.vertices]
    nv = len(hull_verts)

    # Find k hull vertices forming the maximum-volume simplex (most spread-out archetypes)
    best_vol = -1
    best_idx = None
    for combo in combinations(range(nv), k):
        if k == 3:
            # Triangle area = 0.5 * |cross product|
            v0, v1, v2 = hull_verts[combo[0]], hull_verts[combo[1]], hull_verts[combo[2]]
            vol = 0.5 * np.abs(np.cross(v1 - v0, v2 - v0))
        elif k == 2:
            vol = np.linalg.norm(hull_verts[combo[0]] - hull_verts[combo[1]])
        else:
            break
        if vol > best_vol:
            best_vol = vol
    ...
    Z_out = hull_verts[list(best_idx)]

    A_out = np.zeros((n, k))
    for i in range(n):
        A_out[i] = simplex_ls(Z_out, X[i])
    err = np.sum((X - A_out @ Z_out) ** 2)

    return Z_out, A_out, err
...
def plot_overview(X_pca, Z, A, hull_arch, evr, cumvar, n90, n95, spe, spe_limit, save_dir='.'):
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))

    ax = axes[0, 0]
    ax.scatter(X_pca[:, 0], X_pca[:, 1], c='steelblue', alpha=0.4, s=25, label='Data (n=60)')
    ...
    plt.tight_layout(); plt.savefig(os.path.join(save_dir, 'pca_archetypes_first_day.pdf'), dpi=300); plt.close()
    print(f"Saved pca_archetypes_first_day.pdf in {save_dir}")
...
def plot_correlations(A, X, param_names, save_dir='.'):
    ...
    fig.suptitle('Archetype Parameter Correlations (BH FDR less than 0.05) — First Day', fontsize=13, fontweight='bold')
    plt.tight_layout(); plt.savefig(os.path.join(save_dir, 'pca_archetypes_corr_first_day.pdf'), dpi=300); plt.close()
    print(f"Saved pca_archetypes_corr_first_day.pdf in {save_dir}")
...
def main():
    ...
    input_file = 'selected_columns_total_data_behaviour_first_day.csv'

    total_data = load_csv(os.path.join(directory, input_file))
    df = total_data.copy()
    metadata_cols = df[['Experiment', 'sex', 'Type', 'Genotype', 'Hierarchy', 'Mice.chips', 'Animal']]
    df = df.drop(columns=['Experiment', 'sex', 'Type', 'Genotype', 'Hierarchy', 'Mice.chips', 'Animal'])
    ...
    # Save archetype mixing coefficients
    df_out = metadata_cols.reset_index(drop=True).copy()
    for j in range(k_arch):
        df_out[f'Archetype_{j+1}'] = A[:, j]
    arch_idx = A.argmax(axis=1) + 1
    df_out['Archetype_number'] = 'Archetype_' + arch_idx.astype(str)
    df_out['Archetype_description'] = [ARCH_LABELS[i] for i in arch_idx]
    out_path = os.path.join(directory, 'archetype_coefficients_first_day.xlsx')
    df_out.to_excel(out_path, index=False)
    print(f"Saved {out_path}")

    hull_arch, area_ratio, closest, ent = archetype_metrics(X_pca, Z, A)

    plot_overview(X_pca, Z, A, hull_arch, evr, cumvar, n90, n95, spe, spe_limit, save_dir=directory)

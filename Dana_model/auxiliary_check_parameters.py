...
# Set to an integer (1-6) to filter df1 to only that day, or None for all days
day_filter = None
...
if day_filter is not None:
    df1 = df1[df1['Days'] == day_filter].reset_index(drop=True)
    print(f"Filtered to day {day_filter}: {len(df1)} rows")
...
filtered_df.to_excel(
    os.path.join(path, "selected_columns_total_data_behaviour_all_days.xlsx"),
    index=False
)

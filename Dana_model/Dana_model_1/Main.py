...
from plot_utils import (setup_figure, draw_triangle, plot_confusion_matrices, save_if_best,
    plot_aggregate_confusion, plot_aggregate_confusion_mean_se, plot_permutation_tests,
    plot_aggregate_permutation,
...
from main_utils import (
    ...
    compute_mean_normalized_confusion_with_se,
)
...
def main():
    ...
    if  exclude_pca_data:
        no_pca_data = pd.read_excel(f'{directory_path}/{'Data_pca_to_remove.xlsx'}')
    ...
    while state['iteration'] < 50:
        state['attempts'] += 1
        pca_coords, archetypes, varexlp, counts, sample_indices = sample_and_fit_archetypes(all_pca_coords, sample_frac, no_pca_data)
        ...
        if all(c >= min_per_vertex for c in counts):
            ...
            if  exclude_pca_data:
               ...
               matches = np.isclose(all_pca_coords[:, None, :],coords_to_remove[None, :, :]).all(axis=2).any(axis=1)
               pca_coords_filtered = all_pca_coords[~matches]
               all_pca_coords_plot = pca_coords_filtered
               draw_triangle(ax_tri, all_pca_coords_plot, archetypes, counts, varexlp,
            ...
            pvalues = plot_permutation_tests(perm_axes, loocv_results, aligned_true,
                                             hormones_arch, metadata_cols, model_names=user_models)
            ...
            for m in shap_results:
                ...
                state['aggregated_shap'][m]['raw_features'].append(
                    X_full if m in ('RandomForest', 'ExtraTrees', 'HistGB', 'XGBoost') else X_scaled)
    ...
    agg_cm_norm_stats = compute_mean_normalized_confusion_with_se(state['all_true'], state['all_preds_by_model'])
    plot_aggregate_confusion_mean_se(agg_cm_norm_stats, directory_path)

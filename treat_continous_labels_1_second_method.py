    ...
    def cv_pipeline(self, X, y):
        ...
        for train_idx, test_idx in splits:
            ...
            if self.compare_with_baseline:
                ...
                baseline_js_list.append(baseline_js)

                self.baseline_each_archetype(y_train, y_test[0])
                
            print(index)
            index += 1
           # print(f"LOOCV Progress: {index}/{len(X)}")
        if self.compare_with_baseline:
            self.plot_compare_baseline(preds_rmse_list, baseline_rmse_list, self.output_dir, "rmse")
            self.plot_compare_baseline(preds_js_list, baseline_js_list, self.output_dir, "js divergence")
            self.plot_each_archetype_comparison(self.rmse_dict, self.baseline_dict, self.output_dir, "rmse")
...
############Calculate baseline to compare with the model############
    def calculate_baseline_rmse(self, y_train, y_test):
       ...
       # print(f"Baseline RMSE: {baseline_rmse:.4f}")
        return baseline_rmse, baseline_js

#plot compare with the baseline
    def plot_compare_baseline(self, preds_metric_list, baseline_metric_list, output_dir, metric_name = None):
            plt.figure(figsize=(8,6))
            plt.scatter(baseline_metric_list, preds_metric_list, alpha=0.7)

            # Add y = x line
            min_val = min(min(baseline_metric_list), min(baseline_metric_list))
            max_val = max(max(baseline_metric_list), max(baseline_metric_list))
            plt.plot([min_val, max_val], [min_val, max_val], 'r--', label='y = x')
            plt.xlabel('Baseline ' + (metric_name))
            plt.ylabel('Model ' + (metric_name))
            plt.savefig(os.path.join(self.output_dir, (metric_name + '_comparison.pdf')))
            plt.close()
...
#########plot for each archetype the comparison between model and baseline
    def plot_each_archetype_comparison(self, rmse_dict, baseline_dict, output_dir, metric_name):
        ...
        for k in range(n_labels):
            ax = axes[k]
            ax.scatter(baseline_dict[k+1], rmse_dict[k+1], alpha=0.7)
            min_val = min(min(baseline_dict[k+1]), min(rmse_dict[k+1]))
            max_val = max(max(baseline_dict[k+1]), max(rmse_dict[k+1]))
            ax.plot([min_val, max_val], [min_val, max_val], 'r--', label='y = x')
            ax.set_xlabel('Baseline ' + metric_name)
            ax.set_ylabel('Model ' + metric_name)
            ax.set_title(archetypes[k])

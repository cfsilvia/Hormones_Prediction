...
class treat_data_personality:
    def __init__(self,data,select_pairs): 
         self.data = data 
    ...
    def label_encoded(self,ydata):
      ...
      if len(y_unique_values) == 2:
         custom_mapping = {'Arch1' : 0, 'Arch4' : 1, 'Arch2' : 0, 'Arch3' : 1}
    ...
    def get_dictionary_with_features(data_dict, features,indices_dict,frequencies):
      ...
      for i,lst in enumerate(features):
        ...
        results[t]['prob'] = treat_data_personality.get_selected_data(data_dict['prob'],indices_dict[t])
        results[t]['labels_pred'] = treat_data_personality.get_selected_data(data_dict['labels_pred'],indices_dict[t])
        results[t]['confusion_matrix'] = treat_data_personality.get_selected_data(data_dict['confusion_matrix'],indices_dict[t])
        results[t]['accuracy'] = treat_data_personality.get_selected_data(data_dict['accuracy'],indices_dict[t])
        results[t]['precision'] = treat_data_personality.get_selected_data(data_dict['precision'],indices_dict[t])
        results[t]['recall'] = treat_data_personality.get_selected_data(data_dict['recall'],indices_dict[t])
        results[t]['fscore'] = treat_data_personality.get_selected_data(data_dict['fscore'],indices_dict[t])
